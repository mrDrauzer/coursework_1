...
Выберите необходимый пункт меню Получить информацию о транзакциях из:"""
    ...
    while options:
        ...
        if int(choice) not in options:
            print("Такой вариант отсутствует. Выберите другой вариант.")
            continue
...
Доступные для фильтровки статусы:
    ...
    3.PENDING  [ОЖИДАЕТСЯ] """
        ...
        if filter_choice in ("1.0", "1", "ДА", "YES"):
            ...
            list_transac = [
                t
                for t in list_transac
                if isinstance(t, dict) and re.search(keyword, t.get("description", ""), re.IGNORECASE)
            ]
